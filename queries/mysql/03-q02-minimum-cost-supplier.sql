...
SELECT
    ...
    s.s_phone,
    s.s_comment
FROM sr_mysql.testdb.part p
JOIN sr_mysql.testdb.partsupp ps ON ps.ps_partkey = p.p_partkey
JOIN sr_mysql.testdb.supplier s ON s.s_suppkey = ps.ps_suppkey
JOIN sr_mysql.testdb.nation n ON n.n_nationkey = s.s_nationkey
JOIN sr_mysql.testdb.region r ON r.r_regionkey = n.n_regionkey
WHERE p.p_size = 15
