#!/usr/bin/env python3
"""StarRocks JDBC vs ADBC reproducible benchmark CLI (external StarRocks target).

Creates separate JDBC (``bench_jdbc``) and ADBC (``bench_adbc``) catalogs against
the external StarRocks instance (sr-external), runs matched TPC-H queries against
both with EXPLAIN ANALYZE, parses Summary.TotalTime and per-scan-node times, and
prints a wide ASCII comparison table with AVG and GEOMEAN summary rows.

JDBC connects via MySQL wire protocol on port 9030, ADBC via Arrow Flight on
port 9408 — both querying the same ``tpch`` database pre-loaded with SF1 data.
...
Prerequisite: ``docker compose up --build`` with the StarRocks JAR baked in
and sr-external Arrow Flight server running (ports 9030 and 9408).

Usage:
    .venv/bin/python benchmark/starrocks-jdbc-vs-adbc.py
    .venv/bin/python benchmark/starrocks-jdbc-vs-adbc.py --queries 1,3,5 --runs 3
    .venv/bin/python benchmark/starrocks-jdbc-vs-adbc.py --timeout 120 --queries all
...
# External StarRocks instance (sr-external) access:
# JDBC: MySQL wire protocol on port 9030
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="StarRocks JDBC vs ADBC benchmark — TPC-H comparison",
    )
    parser.add_argument(
        "--scale",
        default="sf1",
...
def run_benchmark(args: argparse.Namespace) -> bool:
    ...
    try:
        ...
        print(" StarRocks JDBC vs ADBC Benchmark")
        print(
